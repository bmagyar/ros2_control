...
using transmission_interface::parseTransmissionsFromURDF;

TEST_F(TestTransmissionParser, successfully_parse_valid_urdf)
{
  const auto transmissions = parseTransmissionsFromURDF(valid_urdf_xml_);
...
{
  ASSERT_THROW(parseTransmissionsFromURDF(""), std::runtime_error);
}
...
TEST_F(TestTransmissionParser, empty_urdf_returns_empty)
{
  ...
  const auto transmissions = parseTransmissionsFromURDF(empty_urdf);
  ASSERT_THAT(transmissions, IsEmpty());
}

TEST_F(TestTransmissionParser, wrong_urdf_throws_error)
{
  ASSERT_THROW(parseTransmissionsFromURDF(wrong_urdf_xml_), std::runtime_error);
}
